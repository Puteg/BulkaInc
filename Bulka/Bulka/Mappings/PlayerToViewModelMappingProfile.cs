    ...
    public class PlayerToViewModelMappingProfile : Profile
    {
        public PlayerToViewModelMappingProfile()
        {
            ...
            CreateMap<List<PlayerSession>, PlayerSessionListViewModel>()
                ...
                .ForMember(g => g.AvgInput, map => map.MapFrom(vm => (vm.Sum(c => c.Input)/vm.Count).ToString("0.##")))
                .ForMember(g => g.AvgOutput, map => map.MapFrom(vm => (vm.Sum(c => c.Output)/vm.Count).ToString("0.##")))
                .ForMember(g => g.Profit, map => map.MapFrom(vm => (vm.Sum(c => c.Output) - vm.Sum(c => c.Input)).ToString("0.##")));

            CreateMap<PlayerSession, PlayerSessionItemViewModel>()
                .ForMember(g => g.DateTime, map => map.MapFrom(vm => vm.Begin.ToLongDateString()))
                .ForMember(g => g.Input, map => map.MapFrom(vm => vm.Input.ToString("0.##")))
                .ForMember(g => g.Output, map => map.MapFrom(vm => vm.Output.ToString("0.##")))
